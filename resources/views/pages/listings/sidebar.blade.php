<div class="sidebar">
    ...
    <div class="card-body">
            ...
            {!! Form::open(array('url' => 'listings/','method'=>'get','class'=>'','id'=>'search','role'=>'form')) !!}
            ...
            <div class="form-group">
                <select class="select-picker" name="location_id" data-width="100%" data-live-search="true">
                    ...
                    @foreach(\App\Models\Location::orderBy('location_name')->get() as $location)
                        <option value="{{$location->id}}" @if(isset($_GET['location_id']) AND $_GET['location_id']==$location->id) Selected @endif>{{$location->location_name}}</option> 
                    @endforeach
                </select>
            </div>
