<!-- ================================
     Start Header Area
================================= -->
<header class="nav-header-area border-bottom border-bottom-gray">
            ...
            <div class="nav-main-nav-header-wrap">
                <div class="logo">
                 <a href="{{ URL::to('/') }}" title="logo">   
                  @if(getcong('site_logo'))
                    <img src="{{ URL::asset('upload/'.getcong('site_logo')) }}" alt="logo" style="max-width: 180px;" title="logo">
                  @else
                    <img src="{{ URL::asset('site_assets/images/logo_white.png') }}" alt="logo" style="max-width: 180px;" title="logo">
                  @endif    
                  </a>
                ...
                <div class="nav-right-content ml-auto d-flex align-items-center">
                    <div class="author-access-list mr-3">
                        <a href="{{ URL::to('submit_listing') }}" class="primary_item_btn" title="{{trans('words.add_listing')}}"><span class="fal fa-plus-circle"></span>{{trans('words.add_listing')}}</a>                         
                    </div>

                    @if(!Auth::check())
                    <a class="primary_item_btn" href="{{ URL::to('login/') }}" title="{{trans('words.login')}}">{{trans('words.login')}}</a>
                    @endif
